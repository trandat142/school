...
Ví dụ class `SinhVien` chỉ có thuộc tính và phương thức:
...
Class này không có hàm `main`, nên không chạy trực tiếp như chương trình được.
...
Ví dụ tài khoản ngân hàng có `soDu`.
...
Bên ngoài có thể sửa trực tiếp:

```java
taiKhoan.soDu = -100000;
```

Điều này không hợp lệ vì số dư không nên âm.
...
Sau đó thay đổi thông qua phương thức:
...
- `NhanVien` là class cha.
- `NhanVienToanThoiGian` là class con.
- Class con kế thừa từ class cha bằng `extends`.
...
`Cho` và `Meo` đều là `Animal`, đều có tên và biết kêu, nhưng mỗi con kêu khác nhau.
...
có nghĩa là gọi constructor của class cha `NhanVien` để gán họ tên.

---

## 11. Đặc trưng 3: Đa hình
...
Cả `nv1` và `nv2` đều có kiểu khai báo là `NhanVien`.

Nhưng object thật khác nhau:

- `nv1` là `NhanVienToanThoiGian`
- `nv2` là `NhanVienBanThoiGian`
...
- Không cần viết nhiều vòng lặp riêng.
- Dễ mở rộng thêm loại nhân viên mới.
- Code gọn hơn.
...
Ví dụ class `SinhVien`, constructor, `this`, tạo object và gọi hàm hiển thị.

```text
ViDuSanPham.java
```

Bài tập class `SanPham`, tính thành tiền và hiển thị thông tin.

```text
ViDuNhanVien.java
```

Ví dụ kế thừa: `NhanVienToanThoiGian extends NhanVien`.

```text
ViDuAnimal.java
```

Ví dụ kế thừa và ghi đè phương thức: `Animal`, `Cho`, `Meo`.

```text
ViDuDaHinh.java
```

Ví dụ đa hình: danh sách nhân viên, mỗi loại nhân viên tính lương khác nhau.
